...
def train():
    print("=" * 60)
    print("RETRAINING SPECIALIST MODEL (1200 train images)")
    print("=" * 60)
    
    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    NUM_CLASSES = 36
    IMG_DIR = "/workspace/siu_revision/data/coda_dataset/CODA/base-val-1500/images"
    TRAIN_ANN = "/workspace/siu_revision/data/coda_split/train_annotations.json"
    
    dataset = CODACustomDataset(root_dir=IMG_DIR, annotation_file=TRAIN_ANN)
    ...
    save_path = os.path.join(weights_dir, "specialist_retrained.pth")
    ...
    plt.figure()
    plt.plot(range(1, 21), loss_history, marker='o', color='blue', label='Train Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Specialist Retraining Loss Curve')
    plt.grid(True)
    plt.legend()
    loss_plot_path = os.path.join(figures_dir, "retrain_loss.png")
